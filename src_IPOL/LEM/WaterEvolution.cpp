  ...
  for (size_t i = hBegin; i < hEnd; i++) {
    ...
    for (; j < m_width - 8; j+=8) {
      ...
      _mm256_storeu_ps(oW + j, applyMask256_ps(_mm256_cmp_ps(_mm256_loadu_ps(iL + j), zOceanLvl, _CMP_LT_OS), zOceanLvl - _mm256_loadu_ps(iL + j),
        _mm256_max_ps(_mm256_loadu_ps(oW + j) + _mm256_loadu_ps(iT + j) + zRain * _mm256_loadu_ps(iR + j) * zTimeStep, z0)));
    }
#endif
#if defined(__SSE__)
    //! SSE version
    for (; j < m_width - 4; j+=4) {
      ...
      _mm_storeu_ps(oW + j, applyMask_ps(_mm_cmplt_ps(_mm_loadu_ps(iL + j), xOceanLvl), xOceanLvl - _mm_loadu_ps(iL + j),
        _mm_max_ps(_mm_loadu_ps(oW + j) + _mm_loadu_ps(iT + j) + xRain * _mm_loadu_ps(iR + j) * xTimeStep, x0)));
    }
#endif
    //! Normal version
    for (; j < m_width; j++) {
      ...
      if (iL[j] < oceanLvl) {
        ...
        oW[j] = std::max(oW[j] + iT[j] + timeStep * rain * iR[j], 0.f);
      }
    }
  }
}
